...
function transformRaw$1(rawRes, queryObj) {
    ...
    try {
        ...
        var filtered = sentences.filter(function (x) { return x.hasOwnProperty('trans') && x.hasOwnProperty('orig'); });
        obj.result = filtered.map(function (_a) {
            var trans = _a.trans;
            return trans.trim();
        });
    }
...
function sign$2 (text) {
    ...
    var appVersion = "5.0 (Macintosh; Intel Mac OS X 10_13_0) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/61.0.3163.100 Safari/537.36";
    var salt = ts + parseInt(String(10 * Math.random()), 10);
    return {
        client: client,
        salt: salt,
        ts: ts,
        bv: bluemd5(appVersion),
